...
/// <summary>
/// Small trigger component for waypoint cubes that fires an event when a specified NPC touches it.
/// - Use isTrigger = true on the waypoint collider.
/// - You can match by assigning a targetNPC (NPCManager) OR by providing a targetNPCID string.
/// - If both targetNPC and targetNPCID are empty/null, it will fire for any object that has an NPCManager in its parent chain.
/// - onReached is a UnityEvent you can wire in the inspector (call BaybayinManager.StartTask1Public, Kiko methods, etc.)
...
public class WaypointTrigger : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        var col = GetComponent<Collider>();
        if (col == null)
            Debug.LogWarning($"[WaypointTrigger:{name}] No Collider found (expected one).");
        else if (!col.isTrigger)
            Debug.LogWarning($"[WaypointTrigger:{name}] Collider is not set to 'isTrigger'. Recommended: set it to isTrigger.");
    }
    ...
    void OnTriggerEnter(Collider other)
    {
        ...
        if (match)
        {
            if (debugLogs) Debug.Log($"[WaypointTrigger:{name}] Matched NPC '{(npcMgr != null ? npcMgr.name : other.name)}'. Invoking onReached.");
            ...
            {
                Debug.LogWarning($"[WaypointTrigger:{name}] Exception invoking onReached: {ex}");
            }
        ...
        {
            if (debugLogs) Debug.Log($"[WaypointTrigger:{name}] Entered by '{other.name}' but did not match target filters.");
        }
    }
}
