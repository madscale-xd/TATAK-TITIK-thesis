...
public class HANGERHarvest : MonoBehaviour
{
    [Header("Harvest Settings")]
    [Tooltip("Item given to the player when harvested")]
    public string itemName = "Galapong";
    ...
    [Tooltip("Parameterless method name on BaybayinManager to call after harvesting (e.g. 'Task13'). Leave empty to skip calling BayMan.")]
    public string TaskMethodName = "Task15";
    ...
    private bool IsTaskAllowed()
    {
        ...
        if (BayMan == null)
        {
            Debug.LogWarning($"[HANGERHarvest] TaskStarted='{TaskStarted}' but BayMan reference is null. Interaction will remain disabled until BayMan assigned.");
            return false;
        }
        ...
        {
            Debug.LogWarning($"[HANGERHarvest] Exception while checking BayMan.IsTaskStarted('{TaskStarted}'): {ex}. Treating as not started.");
            return false;
        }
    }
    ...
    public void TryHarvest()
    {
        ...
        {
            Debug.LogWarning("[HANGERHarvest] InventoryManager.Instance is null.");
            FloatingNotifier.Instance?.ShowMessage("You can't harvest right now.", Color.red);
            onFailedInteraction?.Invoke();
            return;
        }
        ...
        InventoryManager.Instance.AddItem("KulayDahon", 10);
        ...
        if (disableAfterTrigger)
        {
            if (ActivateAfter != null)
            {
                ...
                catch (Exception ex) { Debug.LogWarning($"[HANGERHarvest] Failed to activate 'ActivateAfter' GameObject: {ex}"); }
            }
            gameObject.SetActive(false);
        }
    }
    ...
    private void TryInvokeBaybayinTask()
    {
        ...
        if (BayMan == null)
        {
            Debug.LogWarning($"[HANGERHarvest] TaskMethodName='{TaskMethodName}' set but BayMan reference is null.");
            return;
        }
        ...
        try
        {
            ...
            if (mi == null)
            {
                Debug.LogWarning($"[HANGERHarvest] BayMan does not contain a method named '{TaskMethodName}'.");
                return;
            }

            if (mi.GetParameters().Length > 0)
            {
                Debug.LogWarning($"[HANGERHarvest] Method '{TaskMethodName}' on BayMan expects parameters. Expected a parameterless method.");
                return;
            }
        ...
        {
            Debug.LogWarning($"[HANGERHarvest] Exception while invoking '{TaskMethodName}' on BayMan: {ex}");
        }
    }
